...
import { sendFirstClickNotification } from "@/lib/server/notifications";
...
export default async function ProfileRedirectPage({ params }: { params: { slug: string } }) {
  ...
  const { data, error } = await supabase.rpc("track_profile_click_and_get_target", {
    p_slug: slug,
    p_referrer: referrer,
  ...
  if (result.status === "inactive") return <TrackingError title="Link inactive" description="This tracking link has been deactivated by its owner." />;
  if (result.status === "missing_target") return <TrackingError title="Missing LinkedIn URL" description="The owner of this link has not configured a LinkedIn profile URL." />;

  if (result.status === "ok" && result.target_url) {
    if (result.should_notify && result.user_id && result.company_id && result.tracking_link_id) {
      await sendFirstClickNotification({ userId: result.user_id, companyId: result.company_id, trackingLinkId: result.tracking_link_id }).catch(() => null);
    }
